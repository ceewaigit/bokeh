...
/**
 * @deprecated Use getExportStrategy instead
 */
export function determineParallelWorkerCount(
  ...
  const result = getExportStrategy(profile, {
    chunkCount,
    totalFrames: 9000, // 5 minutes at 30fps as default
    fps: 30,
    effectiveMemoryGB: profile.availableMemoryGB ?? profile.totalMemoryGB * 0.4,
    videoSizeEstimateGB
  });
...
export function calculateWorkerAllocation(
  ...
  return getExportStrategy(profile, {
    chunkCount,
    totalFrames,
    fps,
    effectiveMemoryGB
...
/**
 * @deprecated Inlined into getExportStrategy
 */
export function computePerWorkerMemoryMB(
  ...
  return Math.max(512, Math.min(2048,
    Math.floor((memoryBudget * 1024) / Math.max(1, workerCount * 3))
  ));
}
...
export function computeWorkerTimeoutMs(
  ...
  const safetyMultiplier = Math.min(100, Math.max(50, chunkPressure * 4));
  const estimatedSeconds = baseSeconds * safetyMultiplier;
  return Math.min(MAX_TIMEOUT_MS, Math.max(MIN_TIMEOUT_MS, Math.round(estimatedSeconds * 1000)));
}
