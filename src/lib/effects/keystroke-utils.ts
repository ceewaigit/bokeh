...
export function computeKeystrokeSegments(
  events: KeyboardEvent[],
  options: Required<KeystrokeEffectData>
): KeystrokeSegment[] {
  const segments: KeystrokeSegment[] = []
  const BUFFER_TIMEOUT = 800
  const SHORTCUT_COMBINE_THRESHOLD = 1000
  const displayDuration = options.displayDuration || 2000
  ...
  const addShortcut = (keyDisplay: string, timestamp: number) => {
    ...
    if (lastSeg && lastSeg.isShortcut
      && (timestamp - lastSeg.charTimestamps[lastSeg.charTimestamps.length - 1]) <= SHORTCUT_COMBINE_THRESHOLD
    ) {
  ...
  for (const event of orderedEvents) {
    ...
    if (shortcut) {
      const keyDisplay = formatModifierKey(key, event.modifiers, options.showModifierSymbols !== false)
      addShortcut(keyDisplay, event.timestamp)
    } else if (isSpecialKey) {
      const keyDisplay = formatSpecialKey(key)
      addShortcut(keyDisplay, event.timestamp)
...
export function getKeystrokeDisplayState(
  ...
  const fadeOutDuration = options.fadeOutDuration || 400
...
export function getKeystrokeFontFamily(preset: KeystrokeStylePreset, fontFamily?: string): string {
  if (preset === 'terminal') {
    return 'ui-monospace, "SF Mono", Monaco, "Cascadia Code", monospace'
  }
...
export function getKeystrokePresetStyle(
  ...
  switch (preset) {
    ...
    case 'minimal':
      return {
        textColor: '#ffffff',
        textShadow: { color: 'rgba(0, 0, 0, 0.6)', blur: 6 * scale, offsetY: 2 * scale },
      }
    case 'terminal':
      return {
    ...
    case 'outline':
      return {
        ...
        borderWidth: 1.5,
        textColor: '#ffffff',
        textShadow: { color: 'rgba(0, 0, 0, 0.5)', blur: 4 * scale, offsetY: 0 },
      }
    case 'default':
    default:
      return {
        backgroundColor: options.backgroundColor || 'rgba(24, 24, 27, 0.9)',
        textColor: options.textColor || '#ffffff',
        boxShadow: { color: 'rgba(0, 0, 0, 0.2)', blur: 8 * scale, offsetY: 2 * scale },
      }
  }
}
