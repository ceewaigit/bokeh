...
export function useRenderDelay(isRendering: boolean): UseRenderDelayResult {
  ...
  const markRenderReady = useCallback((source?: string) => {
    if (!isRendering) return;
    if (renderReadyRef.current) return;

    console.log(`[useRenderDelay] releasing delayRender (source: ${source || 'unknown'})`);
  ...
  const handleVideoReady = useCallback((event: React.SyntheticEvent<HTMLVideoElement>) => {
    ...
    console.log(`[useRenderDelay] Video ready event: ${event.type}, readyState: ${video.readyState}`);
  ...
  useEffect(() => {
    ...
    if (!readyTimeoutRef.current) {
      ...
      console.log('[useRenderDelay] Setting 10s safety timeout for video load');
      readyTimeoutRef.current = setTimeout(() => {
        console.warn('[useRenderDelay] Safety timeout fired - forcing render continue');
