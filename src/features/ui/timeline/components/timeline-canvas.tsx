...
import { useDragPreview } from '@/features/ui/editor/hooks/use-drag-preview'
import { useAssetDragDrop } from '@/features/ui/editor/hooks/use-asset-drag-drop'
...
const TimelineCanvasContent = React.memo(function TimelineCanvasContent({
  ...
  const baseVideoClips = useMemo(
    () => currentProject ? ClipLookup.videoClips(currentProject) : [],
    [currentProject]
  )
  const baseAudioClips = useMemo(
    () => currentProject ? ClipLookup.audioClips(currentProject) : [],
    [currentProject]
  )

  const videoClips = useMemo(
    () => currentProject ? filterVisible(baseVideoClips) : [],
    [currentProject, filterVisible, baseVideoClips]
  )
  const audioClips = useMemo(
    () => currentProject ? filterVisible(baseAudioClips) : [],
    [currentProject, filterVisible, baseAudioClips]
  )


  const baseWebcamClips = useMemo(
  ...
  const allClipsByTrack = useMemo(() => ({
    [TrackType.Video]: baseVideoClips,
    [TrackType.Audio]: baseAudioClips,
    [TrackType.Webcam]: baseWebcamClips
  }), [baseVideoClips, baseAudioClips, baseWebcamClips])

  const clipBlocksByTrack = useMemo(() => ({
    [TrackType.Video]: buildClipBlocks(baseVideoClips),
    [TrackType.Audio]: buildClipBlocks(baseAudioClips),
    [TrackType.Webcam]: buildClipBlocks(baseWebcamClips)
  }), [baseVideoClips, baseAudioClips, baseWebcamClips])



  const getClipsForTrack = useCallback(
    (trackType: TrackType.Video | TrackType.Audio | TrackType.Webcam) => allClipsByTrack[trackType] ?? [],
    [allClipsByTrack]
  )
  ...
  const globalSkipRanges = useMemo(() => {
    ...
    // 2. Map them, replacing the dragged clip with its preview state
    const effectiveClips = allClips.map(clip => {
      // Check if this clip is being dragged (startTimes has an entry for it)
      const newStartTime = dragPreview.startTimes[clip.id]
      if (newStartTime !== undefined) {
        return {
          ...clip,
          startTime: newStartTime, // The new position!
          // We don't change sourceIn/duration during move, only startTime
        }
      }
      return clip
    })
    ...
    for (const clip of effectiveClips) {
      ...
      for (const region of hiddenRegions) {
        ...
        const projected = TimelineDataService.projectSourceRangeToTimeline(region, clip)
        if (projected) {
