...
/**
 * PlayheadService - Manages playhead state during playback
 * 
 ...
 * - This service works with raw, intact clips
 */
export class PlayheadService {
  ...
  static updatePlayheadState(
    ...
    // Timeline-Centric: use raw tracks (no slicing)
    const tracks = project.timeline.tracks
    const currentClipResult = this.findClipAtTime(tracks, currentTime)
  ...
  static findClipAtTime(tracks: Track[], time: number): { clip: Clip; track: Track } | null {
  ...
  static isNearClipBoundary(tracks: Track[], time: number, tolerance: number): boolean {
    for (const track of tracks) {
