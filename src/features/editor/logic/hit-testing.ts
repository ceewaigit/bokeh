...
import type { Effect, PluginEffectData, AnnotationData, WebcamEffectData } from '@/types/project'
...
export function isPositionableEffect(effect: Effect): boolean {
  switch (effect.type) {
    ...
    // Webcam is handled via specific layout logic, but conceptually positionable
...
export function getEffectBounds(
  ...
  switch (effect.type) {
    ...
    case EffectType.Webcam: {
      const webcamData = effect.data as WebcamEffectData
      // Webcam uses a separate layout util that takes container dims
      const layout = getWebcamLayout(webcamData, videoRect.width, videoRect.height)

      // Position is relative to the container 0,0 (top-left)
      // Because videoRect is our container for calculation, we add its offset
      return {
        x: videoRect.x + layout.x,
        y: videoRect.y + layout.y,
        width: layout.size,
        height: layout.size,
        rotation: 0
      }
...
export function hitTestPreviewLayer(
  ...
  if (selectedEffectId) {
    ...
    if (selectedItem && (selectedItem.type === EffectType.Annotation || selectedItem.type === EffectType.Plugin || selectedItem.type === EffectType.Webcam)) {
