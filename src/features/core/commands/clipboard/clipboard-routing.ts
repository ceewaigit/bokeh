...
export enum ClipboardRouteType {
  Zoom = 'zoom',
  Global = 'global',
  Block = 'block'
}
...
export function getClipboardEffectRoute(effectType: EffectType): ClipboardRouteType {
  if (effectType === EffectType.Zoom) return ClipboardRouteType.Zoom
  if (isGlobalEffectType(effectType)) return ClipboardRouteType.Global
  return ClipboardRouteType.Block
}
