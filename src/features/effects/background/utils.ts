...
export async function initializeDefaultWallpaper() {
    ...
    if (wallpaperInitPromise) return wallpaperInitPromise

    wallpaperInitPromise = (async () => {
        try {
            if (typeof window === 'undefined') return

            // If running in Electron, try to load the default wallpaper from preinstalled wallpapers
            if (window.electronAPI?.listPreinstalledWallpapers && window.electronAPI?.loadImageAsDataUrl) {
